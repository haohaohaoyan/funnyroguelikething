	...
	match boss_current_info["state"]:
		...
		"rush":
			...
			if boss_current_info["attack_state"] == "idle":
				...
				# Show enemy attack
				$Enemy/AttackArea/AnimatedSprite2D.play("attack" + ["1", "2"].pick_random() + "windup")
				$Enemy/AttackArea/AnimatedSprite2D.visible = true
				$Enemy/AttackArea.look_at(Game.player_position)
				...
				var current_hit_name = "attack" + ["1", "2"].pick_random()
				for i in range(attack_count):
					...
					current_hit_name = "attack" + ["1", "2"].pick_random()
					$Enemy/AttackArea/AnimatedSprite2D.play(current_hit_name + "windup")
					await get_tree().create_timer(0.1).timeout
					
					# Breakpoint
					if !get_node_or_null("Enemy"): return
					
					# Face again in case they were dodged through
					$Enemy/AttackArea.look_at(Game.player_position)
					
					var distance = distance_base + ((Game.player_position - $Enemy.global_position).length()) * 1.25
					...
					$Enemy/AttackArea/AnimatedSprite2D.play(current_hit_name + "hit")
					...
					attack_tween.tween_callback(func () :
						...
						$Enemy/AttackArea/AnimatedSprite2D.play(current_hit_name + "cooldown"))
						
					await get_tree().create_timer(attack_cooldown).timeout
				
				$Enemy/AttackArea/AnimatedSprite2D.visible = false
